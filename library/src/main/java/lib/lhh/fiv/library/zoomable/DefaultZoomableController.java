...
public class DefaultZoomableController
    ...
    public void setImageBounds(RectF imageBounds) {
        mImageBounds.modify(imageBounds);
    }
    ...
    public Point mapViewToImage(Point viewPoint) {
        ...
        points[0] = viewPoint.getPointX();
        points[1] = viewPoint.getPointY();
    ...
    public Point mapImageToView(Point imagePoint) {
        float[] points = mTempValues;
        points[0] = imagePoint.getPointX();
        points[1] = imagePoint.getPointX();
    ...
    private void mapRelativeToAbsolute(float[] destPoints, float[] srcPoints, int numPoints) {
        for (int i = 0; i < numPoints; i++) {
            destPoints[i * 2 + 0] = srcPoints[i * 2 + 0] * mImageBounds.getWidth() + mImageBounds.left;
            destPoints[i * 2 + 1] = srcPoints[i * 2 + 1] * mImageBounds.getHeight() + mImageBounds.top;
        }
    }
    ...
    public void setTransform(Matrix activeTransform) {
        ...
        mActiveTransform.setMatrix(activeTransform);
    }
    ...
    public void zoomToImagePoint(float scale, Point imagePoint) {
        ...
        float[] points = mTempValues;
        points[0] = imagePoint.getPointX();
        points[1] = imagePoint.getPointY();
        mapRelativeToAbsolute(points, points, 1);
        mActiveTransform.setScale(scale, scale, points[0], points[1]);
        mActiveTransform.postTranslate(
                mViewBounds.getCenter().getPointX() - points[0],
                mViewBounds.getCenter().getPointY() - points[1]);
        limitTranslation();
    }
    ...
    public void onGestureBegin(TransformGestureDetector detector) {
        mPreviousTransform.setMatrix(mActiveTransform);
    }

    @Override
    public void onGestureUpdate(TransformGestureDetector detector) {
        mActiveTransform.setMatrix(mPreviousTransform);
    ...
    public float getScaleFactor() {
        mActiveTransform.getElements(mTempValues);
        return mTempValues[0];
    }
    ...
    private boolean limitTranslation() {
        ...
        bounds.modify(mImageBounds);
        mActiveTransform.mapRect(bounds);

        float offsetLeft = getOffset(bounds.left, bounds.getWidth(), mViewBounds.getWidth());
        float offsetTop = getOffset(bounds.top, bounds.getHeight(), mViewBounds.getHeight());
